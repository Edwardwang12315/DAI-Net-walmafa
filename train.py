...
def train():
    ...
    if not args.resume:
        ...
        net=recon_weights(net,'../model/WalMaFa/WalMaFa_LOL_v1_tiny/models/model_bestPSNR.pth')
        # exit()

    # Scaling the lr
    # 设置了根据批次大小和gpu数量调整学习率的机制
    lr = args.lr * np.round(np.sqrt(args.batch_size / 4 * torch.cuda.device_count()),4)
    optimizer = optim.SGD( dsfd_net.parameters() , lr = lr , momentum = args.momentum , weight_decay = args.weight_decay )
    # param_group = []
    ...
    # param_group += [{'params': dsfd_net.estimator.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.coefficient(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.patch_embed.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.patch_embed_mask.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.sim.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.down_1.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.decoder_level1_0.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.down_2.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.decoder_level2_0.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.down_3.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.decoder_level3_0.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.latent.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.up3_2.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.decoder_level2_1.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.up2_1.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.decoder_level1_1.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.up2_0.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.coefficient_3_2.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.coefficient_2_1.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.coefficient_1_0.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.skip_3_2.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.skip_2_1.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.skip_1_0.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.sim.parameters(), 'lr': lr}]
    # param_group += [{'params': dsfd_net.output.parameters(), 'lr': lr}]
    #
...
def recon_weights( net , path_oriMod ) :
	# 加载预训练权重
	...
	a = ori_module_dict[ 'state_dict' ]
	tar_module_dict = net.state_dict()
	# torch.save(tar_module_dict,'src_mod.pth') # 保存原网络权重文件
 
	# 筛选出名称和结构相同的模块权重
	matched_dict = {
			name : weight for name , weight in a.items()
			if name in tar_module_dict and weight.shape == tar_module_dict[ name ].shape
	}
	# torch.save(matched_dict,'matched_dict.pth') # 保存两网络中相同部分
	...
	# 调试使用，查看生成权重文件
	# tar_module_dict=net.state_dict()
	# torch.save(tar_module_dict,'tar_mod.pth')
	return net
